...
function onMarkerClick(s) {
  ...
  if (!startPt) {
    ...
    document.getElementById('startStation').value = s.name;
  } else if (!endPt) {
    endPt = { lat: s.latitude, lng: s.longitude, name: s.name };
    document.getElementById('endStation').value = s.name;
  } else {
    startPt = { lat: s.latitude, lng: s.longitude, name: s.name };
    endPt = null;
    document.getElementById('startStation').value = s.name;
...
document.getElementById('btnPlan').addEventListener('click', () => {
  if (!startPt || !endPt) { document.getElementById('routeInstructions').textContent = '⚠ Click two stations first.'; return; }
  ...
  routeCtrl = L.Routing.control({
    ...
    routeWhileDragging: false, addWaypoints: false, showAlternatives: true
  }).addTo(map);
  document.getElementById('routeInstructions').textContent = `Route: ${startPt.name} → ${endPt.name}`;
});
